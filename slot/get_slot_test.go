...
func TestGetSlotById(t *testing.T) {
	...
	doctor, err := graphql.CreateDoctor(model.CreateDoctorInput{
		Email:     "test_doctor_get_slot@edgar-sante.fr",
		Password:  "password",
		Name:      "name",
		Firstname: "first",
	...
	if createSlot.Code != 201 {
		t.Errorf("Expected code 201 but got %d", createSlot.Code)
	}
	if createSlot.Err != nil {
		t.Errorf("Expected no error but got: %s", createSlot.Err.Error())
	}

	response := GetSlotById(createSlot.Rdv.ID, doctor.ID)
...
func TestGetSlotByIdInvalidId(t *testing.T) {
	if err := godotenv.Load(".env.test"); err != nil {
		log.Fatalf("Error loading .env.test file: %v", err)
	}

	doctor, err := graphql.CreateDoctor(model.CreateDoctorInput{
		Email:     "test_doctor_get_slot_invalid@edgar-sante.fr",
		Password:  "password",
		Name:      "name",
		Firstname: "first",
		Address: &model.AddressInput{
	...
	response := GetSlotById("test_invalid_id_get_slot", doctor.ID)
...
func TestGetSlots(t *testing.T) {
	...
	doctor, err := graphql.CreateDoctor(model.CreateDoctorInput{
		Email:     "test_doctor_get_slot_all@edgar-sante.fr",
		Password:  "password",
		Name:      "name",
		Firstname: "first",
		Address: &model.AddressInput{
...
func TestGetSlotsInvalidId(t *testing.T) {
	if err := godotenv.Load(".env.test"); err != nil {
		log.Fatalf("Error loading .env.test file: %v", err)
	}
