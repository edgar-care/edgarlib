	...
	if err == nil {
		...
		if methodToRemove == "MOBILE" {
			...
			for _, deviceID := range doubleAuth.TrustDeviceID {
				...
				_, err = graphql.UpdatePatientTrustDevice(deviceID, model.UpdatePatientTrustDeviceInput{
					TrustDevices: remove(doubleAuth.TrustDeviceID, &deviceID),
				})
				if err != nil {
	...
	if err == nil {
		...
		if methodToRemove == "MOBILE" {
			...
			for _, deviceID := range doubleAuth.TrustDeviceID {
				...
				_, err = graphql.UpdateDoctorsTrustDevice(deviceID, model.UpdateDoctorsTrustDeviceInput{
					TrustDevices: remove(doubleAuth.TrustDeviceID, &deviceID),
				})
				if err != nil {
					return errors.New("failed to delete trust_device")
