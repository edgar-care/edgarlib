...
func getDurationSymptom(session *model.Session) *string {
	if session.LastQuestion != "" && session.LastQuestion != "describe medicines" && session.LastQuestion != "describe symptoms" && strings.Split(session.LastQuestion, " | ")[0] == "duration" {
...
func updateSymptomsWithNewData(symptoms []model.SessionSymptom, newSymptoms utils.NlpResponseBody) []model.SessionSymptom {
	for _, s := range newSymptoms.Context {
		if s.Name == "describe medicines" || s.Name == "describe symptoms" {
			continue
		}
