...
func GetSessionDiseases(sessionContext []model.SessionSymptom, imc float64) ([]graphql.SessionDiseasesInput, error) {
	...
	for i, e := range diseases.GetDiseases {
		mapped[i] = exam.CalculPercentage(sessionContext, e, imc)
	}
