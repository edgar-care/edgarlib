...
func ResetPassword(email string, password string, uuid string) ResetPasswordResponse {
	...
	_, err = graphql.UpdatePatient(context.Background(), gqlClient, patient.GetPatientByEmail.Id, patient.GetPatientByEmail.Email, utils.HashPassword(password), patient.GetPatientByEmail.Medical_info_id, patient.GetPatientByEmail.Rendez_vous_ids, patient.GetPatientByEmail.Document_ids, patient.GetPatientByEmail.Treatment_follow_up_ids, patient.GetPatientByEmail.Chat_ids, patient.GetPatientByEmail.Device_connect, patient.GetPatientByEmail.Double_auth_methods_id, patient.GetPatientByEmail.Status)
	if err != nil {
