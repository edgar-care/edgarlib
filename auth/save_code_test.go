...
func TestCreateBackupCodesAdd(t *testing.T) {
	if err := godotenv.Load(".env.test"); err != nil {
		log.Fatalf("Error loading .env.test file: %v", err)
	}

	patient, err := graphql.CreatePatient(model.CreatePatientInput{
		Email:    "test_patient_create_save_code_add@edgar-sante.fr",
		Password: "password",
		Status:   false,
	})
	if err != nil {
	...
	input := double_auth.CreateDoubleAuthInput{
		Methods: "EMAIL",
	}

	_ = double_auth.CreateDoubleAuthEmail(input, patient.ID)
