...
func DeleteMedicalInfo(medicalId string, patientId string) DeleteMedicalInfoResponse {
	...
	updatedPatient, err := graphql.UpdatePatient(context.Background(), gqlClient, patientId, patient.GetPatientById.Email, patient.GetPatientById.Password, "", patient.GetPatientById.Rendez_vous_ids, patient.GetPatientById.Document_ids)
