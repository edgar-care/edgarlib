...
func TestAddMedicalAntecedent(t *testing.T) {
	...
	antecedentInput := CreateNewMedicalAntecedentInput{
		...
		Treatments: []CreateTreatInput{{
			CreatedBy: "test",
			StartDate: 1234, // Timestamp actuel
			EndDate:   5678, // Un jour plus tard
			Medicines: []CreateAntecedentsMedicines{{
				Period: []*CreateAntecedentPeriod{{
					Quantity:       2,
					Frequency:      2,
					FrequencyRatio: 2,
					FrequencyUnit:  "DAY", // Assure-toi que c'est l'énumération correcte
					PeriodLength:   2,
					PeriodUnit:     "DAY", // Assure-toi que c'est l'énumération correcte
					Comment:        "comment",
				}},
			}},
		}},
